...
(use-package pyvenv
  ...
  (pyvenv-mode 1)
  :init
  (defun sergio/modify-pythonpath ()
	(interactive)
	(let* ((pyver (car (s-match "[0-9]+\.[0-9]+" (nth 1 (split-string (string-chop-newline (shell-command-to-string "python --version")) " ")))))
		   (venv-site-packages (f-join pyvenv-virtual-env (concat "lib/python"
									  pyver) "site-packages")))
	  (setenv "PYTHONPATH" venv-site-packages)
	  (message venv-site-packages)))
  (add-hook 'pyvenv-post-activate-hooks 'sergio/modify-pythonpath)
...
(use-package poetry
  :hook
  ((python-mode python-ts-mode) . poetry-tracking-mode))
...
(use-package python
  ...
  ("C-<return>" . (lambda () (interactive) (python-shell-send-statement) (python-nav-forward-statement)))
  :hook
...
(use-package eglot
  ...
  (add-hook 'pyvenv-post-activate-hooks #'(lambda () (interactive) (add-hook 'before-save-hook 'eglot-format-buffer nil t)))
