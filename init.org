...
(leader
  ...
  "pt" '((lambda ()
	   (interactive)
	   (when (get-buffer "*pytest*")
	     (kill-buffer "*pytest*"))
	   (let ((default-directory (poetry-find-project-root)))
	     (when default-directory
	       (defun recenter-temp-buffer (process event)
		 (pop-to-buffer (process-buffer process)) (recenter-top-bottom))
	       (set-process-sentinel
		(start-process "pytest" "*pytest*" "poetry" "run" "pytest" ".")
		'recenter-temp-buffer)))) :which-key "PyTest"))
